...
use super::db::{JsonDb, random_id};
use super::schemas::{Todo, JsonError};
...
async fn get_todo(web::Path(id): web::Path<i32>, db: web::Data<JsonDb>) -> impl Responder {
	...
	match wanted_todo {
		...
		None => {
			let json_error = serde_json::to_string(&JsonError { error: "Todo not found".to_string()}).unwrap();
...
async fn post_todo(db: web::Data<JsonDb>, mut todo: web::Json<Todo>) -> impl Responder {
	...
	*db_string = updated_db_string.clone();
	...
	//write in db.json
	web::block( || {
		fs::write("./db.json", updated_db_string)
	}).await.unwrap();
